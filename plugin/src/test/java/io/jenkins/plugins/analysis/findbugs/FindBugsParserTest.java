...
class FindBugsParserTest {
    ...
    private Issues<Issue> parseFile(final String fileName, final boolean isRankActivated) {
        return new FindBugsParser(isRankActivated).parse(getInputStreamProvider(fileName),
    ...
    private InputStream read(final String fileName) {
        return hudson.plugins.findbugs.parser.FindBugsParserTest.class.getResourceAsStream(fileName);
    }
    ...
    void testMessageMapping() throws Exception {
        try (InputStream stream = read(FINDBUGS_NATIVE_XML)) {
            ...
            assertThat(mapping).containsKeys(FIRST_WARNING, SECOND_WARNING);
            assertThat(mapping.get(FIRST_WARNING)).isEqualTo("Inconsistent synchronization of org.apache.hadoop.dfs.BlockCrcUpgradeObjectDatanode.blocksPreviouslyUpgraded; locked 85% of time");
            assertThat(mapping.get(SECOND_WARNING)).isEqualTo("Should org.apache.hadoop.streaming.StreamJob$MultiPropertyOption be a _static_ inner class?");
        }
    }
   ...
   /**
    ...
    private void scanNativeFile(final String findbugsFile, final String projectName,
        ...
        assertThat(issues).hasSize(NUMBER_OF_WARNINGS);
